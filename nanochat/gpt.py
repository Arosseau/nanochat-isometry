...
class GPT(nn.Module):
    ...
    def _compute_window_sizes(self, config):
        ...
        short_window = -(-long_window // 4 // 128) * 128  # ceil to FA3 tile size (2048 -> 640)
        char_to_window = {
            "L": (long_window, 0),
            "S": (short_window, 0),
        }
    ...
    def estimate_flops(self):
        ...
        # Exclude non-matmul params: embeddings, per-layer scalars, LayerNorm params, smear/backout
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02, weight_decay=0.0, scalar_lr=0.5, optimizer_type='muon-adamw'):
        ...
        assert len(list(self.parameters())) == (
            ...
            len(value_embeds_params) + len(resid_params) + len(x0_params) + len(all_norm_params) + len(smear_params)
        )
