#!/bin/bash
#SBATCH --job-name=nanochat-d24-1gpu-base-oi
...
# orthogonally (all singular values = 1) instead of uniform/Xavier.
# Isolates the effect of initialization alone (no ortho regularization during training).
#
# Usage:
#   sbatch runs/h200_d24_1gpu_baseline_orthoinit.sh
#   bash runs/h200_d24_1gpu_baseline_orthoinit.sh
#   SERIES_NAME=myexp sbatch runs/h200_d24_1gpu_baseline_orthoinit.sh
...
DEPTH=24
TAG="${SERIES_NAME}_d24_1gpu_baseline_orthoinit"

RESULTS_DIR="$NANOCHAT_BASE_DIR/${SERIES_NAME}_isometry_results"
mkdir -p "$RESULTS_DIR"
LOG="$RESULTS_DIR/${TAG}.log"

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Running d${DEPTH} baseline + ortho-init (1×H200, FP8)"
START=$(date +%s)
...
python -m scripts.base_train \
    ...
    --fp8 \
    --run="${SERIES_NAME} baseline wd=0.2 ortho-init d24" \
    --model-tag="${TAG}" \
    --weight-decay=0.2 \
    --ortho-init \
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] d24_1gpu_baseline_orthoinit: bpb=$VAL_BPB, time=${ELAPSED}s"

RESULTS_FILE="$RESULTS_DIR/results.csv"
[ ! -f "$RESULTS_FILE" ] && echo "name,val_bpb,train_time_sec" > "$RESULTS_FILE"
echo "d24_1gpu_baseline_orthoinit,$VAL_BPB,$ELAPSED" >> "$RESULTS_FILE"
