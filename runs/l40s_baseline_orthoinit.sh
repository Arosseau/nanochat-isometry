#!/bin/bash
#SBATCH --job-name=nanochat-l40s-base-oi
...
#
# Usage:
#   sbatch runs/l40s_baseline_orthoinit.sh
#   bash runs/l40s_baseline_orthoinit.sh
#   SERIES_NAME=myexp sbatch runs/l40s_baseline_orthoinit.sh
...
DEPTH=12
TAG="${SERIES_NAME}_l40s_baseline_orthoinit"

RESULTS_DIR="$NANOCHAT_BASE_DIR/${SERIES_NAME}_isometry_results"
mkdir -p "$RESULTS_DIR"
LOG="$RESULTS_DIR/${TAG}.log"

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Running d${DEPTH} baseline + ortho-init (1×L40S, BF16)"
START=$(date +%s)

python -m scripts.base_train \
    --depth=$DEPTH \
    --run="${SERIES_NAME} baseline wd=0.2 ortho-init" \
    --model-tag="${TAG}" \
    --weight-decay=0.2 \
    --ortho-init \
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] l40s_baseline_orthoinit: bpb=$VAL_BPB, time=${ELAPSED}s"

RESULTS_FILE="$RESULTS_DIR/results.csv"
[ ! -f "$RESULTS_FILE" ] && echo "name,val_bpb,train_time_sec" > "$RESULTS_FILE"
echo "l40s_baseline_orthoinit,$VAL_BPB,$ELAPSED" >> "$RESULTS_FILE"
